...
namespace LibreCanvas {
    ...
    void CloneStampTool::onMousePress(QMouseEvent *event, std::shared_ptr<Document> doc, const QPoint& imagePos)
    {
        ...
        cloneBrush(painter, m_sourcePoint, imagePos, m_size, m_hardness, m_opacity);
        painter.end();
        
        doc->saveState("Clone Stamp");
    }
    ...
    void CloneStampTool::cloneBrush(QPainter& painter, const QPoint& sourcePos, const QPoint& destPos, int size, float hardness, float opacity)
    {
        const QImage& sourceImage = painter.device()->property("sourceImage").value<QImage>();
